...
def cria_livro(id_livro, descricao):
    with engine.connect() as con:
        sql_create = text(
            "INSERT INTO (id_livro, descricao) VALUES (:id_livro, :descricao)")
...
def devolve_livro(id_livro):
    with engine.connect() as con:
        sql_update = text(
            "UPDATE livro SET id_aluno = NULL WHERE id_livro = :id_livro")
...
def livros_do_aluno(nome):
    with engine.connect() as con:
        sql_consulta = text(
            "SELECT id_livro, id_aluno, descricao FROM livro JOIN aluno ON livro.id_aluno = aluno.id WHERE aluno.nome = :nome")
